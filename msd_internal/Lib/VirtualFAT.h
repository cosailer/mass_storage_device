...
#define _VIRTUALFAT_H_
    ...
    #define LUN_MEDIA_BLOCKS          (FILE_SECTORS(FLASH_FILE_SIZE_BYTES))
    ...
    //static void WriteEEPROMByte(uint8_t* const Address, const uint8_t Data) ATTR_NO_INLINE;

    //static void UpdateFAT12ClusterEntry(uint8_t* const FATTable, const uint16_t Index, const uint16_t ChainEntry) AUX_BOOT_SECTION;

    //static void UpdateFAT12ClusterChain(uint8_t* const FATTable, const uint16_t StartIndex, const uint8_t ChainLength) AUX_BOOT_SECTION;
